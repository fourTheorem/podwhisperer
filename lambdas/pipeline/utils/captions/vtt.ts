 ...
 * Creates one cue per word with the highlighted word, plus filler cues for timing gaps.
...
function generateHighlightedCues(
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    // Insert filler cue if there's a gap between last end and current word start
    ...
    // Add highlighted cue for this word
    cues.push({
      start: wordStart,
      end: wordEnd,
      text:
...
/**
 * Generate cues for a segment without word highlighting (basic mode).
 */
 ...
 */
export function generateVtt(options: GeneratorOptions): string {
